 ...
 */
function velocity_form_install_configure_form_alter(&$form, $form_state) {
  // Pre-populate the site name with the server name.
  $form['site_information']['site_name']['#default_value'] = $_SERVER['SERVER_NAME'];
  $form['#submit'][] = 'velocity_form_submit';
}


function velocity_form_submit($form, &$form_state) {
  // Import database velocity file.
  $velocity_file = dirname(__FILE__) . '/velocity.sql';
  $success = import_velocity($velocity_file);
...
/**
 * Imports a database Velocity file.
 *
 * @see demo_reset().
 */
function import_velocity($filename) {
  ...
  if (!file_exists($filename) || !($fp = fopen($filename, 'r'))) {
    drupal_set_message(t('Unable to open Velocity file %filename.', array('%filename' => $filename)), 'error');
    return FALSE;
  }
